...
function App() {
  ...
  const [tempType, setTempType] = useState("Veg");
  const [tempSize, setTempSize] = useState("Large");
  const [tempBase, setTempBase] = useState("Thin");
  ...
  const handleBaseChange = (event) => {
    setTempBase(event.target.value);
  };
  ...
  const handleMoveToNextStage = (orderId) => {
    const updatedOrders = orders.map((order) =>
      order.id === orderId
        ? { ...order, stage: order.stage + 1, timeInStage: 0 }
        : order
    );
  ...
  useEffect(() => {
    const deliveredToday = orders.filter(
      (order) => order.stage === stages.length - 1
    ).length;
    setTotalDelivered(deliveredToday);
  }, [orders]);

  const sortedOrders = [...orders].sort((a, b) => {
    const stageDelayA = a.timeInStage - sizeMakingTime[a.size];
    const stageDelayB = b.timeInStage - sizeMakingTime[b.size];
  ...
  return (
    <Card sx={{ backgroundColor: "#e6f2ff", padding: "1rem" }}>
      <div>
      ...
      >
        <CardContent>
          ...
          >
            Total Pizza Delivered Today:{totalDelivered}
          </Typography>

          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <FormLabel component="legend">Select Type Of Pizza</FormLabel>
              <RadioGroup
                row
                aria-label="type"
                name="type"
            ...
            <Grid item xs={12} sm={6}>
              <FormLabel component="legend">Select Pizza Size</FormLabel>
              <RadioGroup
                row
            ...
            <Grid item xs={12} sm={6}>
              <FormLabel component="legend">Select Base of Pizza</FormLabel>
              <RadioGroup
                row
                aria-label="base"
                name="base"
      ...
      <div>
        <Typography variant="h5">Pizza Stages</Typography>
        <Grid container spacing={2}>
                ...
                {sortedOrders
                  ...
                  .map((order) => (
                    <Card
                      ...
                      style={{
                        backgroundColor:
                          order.timeInStage > sizeMakingTime[order.size]
                            ? "red"
                            : "white",
                      }}
                    >
                      <CardContent>
                        ...
                        <Typography variant="body2">
                          Time in Stage: {order.timeInStage} min
                        </Typography>
